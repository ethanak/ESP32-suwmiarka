...
uint16_t Lektor2::getUnichar(char **str)
{
    uint16_t znak;
    char n;
    unsigned char m;
...
int Lektor2::uniIsspace(int znak)
{
	int i;
...
int Lektor2::_pushout(char c, char **strout, char *strin, unsigned char *blank)
{
    if (c == ' ') {
        *blank = 1;
        return 0;
...
int Lektor2::_cpy(int n, char **strout, char *strin, unsigned char *blank)
{
    char znak;
    for (;;) {
...
int Lektor2::toISO2(char *buffer, int buflen)
{
...
int Lektor2::isoVowel(unsigned char znak)
{
    if (pgm_read_byte(&letterflags[znak]) & 4) return 1;
    return 0;
}

int Lektor2::isoAlnum(unsigned char znak)
{
    if (pgm_read_byte(&letterflags[znak]) & 9) return 1;
    return 0;
}
int Lektor2::isoAlpha(unsigned char znak)
{
    if (pgm_read_byte(&letterflags[znak]) & 1) return 1;
    return 0;
}
int Lektor2::isoUpper(unsigned char znak)
{
    if (pgm_read_byte(&letterflags[znak]) & 2) return 1;
    return 0;
}
int Lektor2::isoToLower(unsigned char znak)
{
